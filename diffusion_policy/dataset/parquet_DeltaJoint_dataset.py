...
@dataclass
class _EpisodeCache:
    q_curr: np.ndarray       # (N, 6)
    q_tgt: np.ndarray        # (N, 6)
...
class ParquetDeltaJointDataset(BaseLowdimDataset):
    ...
    最终 __getitem__ 返回：
        obs    : (horizon, obs_dim)  # 6 + 7 + 7 = 20
        action : (horizon, action_dim) # 6
    ...
    def __init__(
        ...
        # LeRobot 布局: data_dir 为含 chunk-000,chunk-001,... 的父目录，且 scan_chunk_subdirs=True
        ...
        assert len(episode_paths_all) > 0, (
            f"No parquet found. data_dir={self.data_dir!r}, episode_glob={self.episode_glob!r}, "
        ...
        for ep_id, p in enumerate(self.episode_paths):
            ...
            if self.cache_in_memory:
                # 强行切片：抛弃最后维度的夹爪数据
                q_curr = np.stack(df[self.q_curr_key].to_numpy(), axis=0).astype(np.float32)[:, :6]
                q_tgt = np.stack(df[self.q_tgt_key].to_numpy(), axis=0).astype(np.float32)[:, :6]
                eef_curr = np.stack(df[self.eef_curr_key].to_numpy(), axis=0).astype(np.float32)[:, :7]
                eef_tgt = np.stack(df[self.eef_tgt_key].to_numpy(), axis=0).astype(np.float32)[:, :7]

                # 强制四元数半球对齐
                eef_curr = self._align_quaternions(eef_curr)
                eef_tgt = self._align_quaternions(eef_tgt)
    ...
    def _align_quaternions(pose_array: np.ndarray, w_idx: int = 6, quat_start: int = 3) -> np.ndarray:
        """
        强制四元数半球对齐，避免双倍覆盖导致的网络拟合抖动。
        pose_array: (N, 7) 的位姿数组
        w_idx: 实部 w 的索引（基于你的数据分布，w在第6个）
        quat_start: 四元数部分开始的索引（基于你的数据，quat部分是[3,4,5,6]）
        """
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        q_curr = ep.q_curr[start:start + T]       # (T, 6)
        q_tgt = ep.q_tgt[start:start + T]         # (T, 6)
        ...
        return {
            "obs": torch.from_numpy(obs).float(),         # (T, 20)
            "action": torch.from_numpy(delta_q).float(),  # (T, 6)
        }
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        for i in sel:
            item = self[i]
            obs_list.append(item["obs"].numpy())      # (T, 20)
            act_list.append(item["action"].numpy())   # (T, 6)

        obs_arr = np.concatenate(obs_list, axis=0)    # (sumT, 20)
        act_arr = np.concatenate(act_list, axis=0)    # (sumT, 6)
